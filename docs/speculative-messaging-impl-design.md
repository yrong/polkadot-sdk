...
- **Collator-side Late Block Proof collection not yet implemented.** The POC
  ...
  Without collator-side LBP collection, the candidate is submitted with the
  ...
  `validate_block/implementation.rs` — is fully intact. The natural integration
  point is inside the lookahead collator's post-build block in `lookahead.rs`:
  after calling `compute_provides_root`/`requires_commitments`, compare each
  `requires` root against `relay_client.provides_root(source, relay_parent)`;
  fetch and apply `LateBlockProof`s for any that have advanced, then override
  `collation.requires` with the transformed roots before submitting.
