...
//! on-chain via `SourceDiscovery::set_source_genesis` (the source's genesis hash),
...
use penpal::runtime_types::{
	cumulus_pallet_source_discovery::pallet::Call as SourceDiscoveryCall,
...
async fn set_source_genesis(
	...
	let call = SourceDiscoveryCall::set_source_genesis {
		source: PenpalId(source),
		info: Some((genesis, None)),
	};
	let tx = penpal::tx().sudo().sudo(PenpalRuntimeCall::SourceDiscovery(call));
	client
		.tx()
		.sign_and_submit_then_watch_default(&tx, &dev::alice())
		.await?
