...
pub struct MessageBatch {
	...
	/// Total number of MMR nodes (positions, not leaves) in the per-destination subtree
	/// at the moment this batch was generated. Required to verify the batch inclusion
	/// proof against `subtree_root`.
	pub subtree_mmr_size: u64,
	/// Combined MMR inclusion proof over every leaf in `messages`, against `subtree_root`.
	/// Generated by `mmr_lib::MMR::gen_proof` over the leaf positions of all included
	/// messages and verified by `MerkleProof::verify(subtree_root, leaves)` where `leaves`
	/// is `(leaf_index_to_pos(position), Keccak256(payload))` for each message.
