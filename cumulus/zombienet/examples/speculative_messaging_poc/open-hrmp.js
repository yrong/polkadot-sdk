...
async function main() {
  ...
  const relayWs  = process.env.RELAY_WS    || 'ws://127.0.0.1:9900';
  ...
  console.log(`Connected: ${(await api.rpc.system.chain()).toString()} @ ${relayWs}`);

  const keyring = new Keyring({ type: 'sr25519' });
  const sudo = keyring.addFromUri(sudoSeed);
  console.log(`Sudo: ${sudo.address}`);

  for (const [sender, recipient] of [[paraA, paraB], [paraB, paraA]]) {
    console.log(`\nOpening ${sender} → ${recipient}…`);
    await signAndWait(
      api,
      sudo,
      api.tx.sudo.sudo(api.tx.hrmp.forceOpenHrmpChannel(sender, recipient, maxCap, maxSize)),
    );
  }
  ...
  const present    = ch => typeof ch?.isSome === 'boolean' ? ch.isSome : ch?.toString() !== '0x';
  ...
  while (true) {
    ...
    if (n - startBlock >= maxBlocks)
      throw new Error(`HRMP channels not present after ${maxBlocks} relay blocks`);
