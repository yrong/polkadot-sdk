	...
	RuntimeApi: ConstructNodeRuntimeApi<Block, ParachainClient<Block, RuntimeApi>>,
{
	...
	/// consensus wiring.
	///
	/// The fetch pipeline reads source peers from the shared [`PeerRegistry`]
	/// populated by the standalone `cumulus-client-source-discovery` worker
	/// (`SourceDiscoveryApi::source_discovery_info()`, governance-set on-chain).
	pub(crate) fn start(
		self,
