...
pub enum ParachainBlockData<Block> {
	...
	/// V3 adds the speculative-messaging lifts: the PoV-carried proofs `validate_block` verifies
	/// to synthesize the candidate's `Requires` UMP signal from the blocks' consumption records.
	///
	/// Like `scheduling_proof`, lifts are candidate-assembly data supplied by the submitting
	/// collator, never part of the blocks. They can be regenerated by anyone from public data, so
	/// a block never goes stale: a resubmission regenerates them against the current provides.
	V3 {
		blocks: Vec<Block>,
		proof: CompactProof,
		/// Required when the runtime has V3 scheduling enabled, exactly as choosing V2 over V1
		/// is; `validate_block` rejects a V3-scheduling candidate without one. Lifts do not
		/// imply V3 scheduling, so it stays optional here.
		...
		if prefix == VERSIONED_PARACHAIN_BLOCK_DATA_PREFIX {
			match input.read_byte()? {
				...
				3 => {
					...
					// `LiftsBySource::decode` enforces the canonical form at the untrusted
					// boundary: strictly increasing `ParaId`s, bounded source count.
...
impl<Block> ParachainBlockData<Block> {
	...
	/// A V3 PoV, carrying speculative-messaging lifts.
	...
	pub fn into_inner(self) -> (Vec<Block>, CompactProof) {
		match self {
			...
			Self::V2 { blocks, proof, .. } | Self::V3 { blocks, proof, .. } => (blocks, proof),
		}
	}
...
mod tests {
	...
	fn decoding_encoding_v3_works() {
		use cumulus_primitives_spec_messaging::{MMRExtensionProof, RequiresLift, StreamProof};
		...
		let lifts = LiftsBySource::try_from(BTreeMap::from([
			(ParaId::from(1000), vec![lift()]),
			(ParaId::from(2000), vec![lift(), lift()]),
		]))
		.unwrap();
		let scheduling_proof = crate::SchedulingProof {
			header_chain: vec![make_relay_header(5)],
		...
		// Lifts do not imply V3 scheduling: with and without a scheduling proof.
		...
		// Earlier versions carry no lifts.
		let v1 = ParachainBlockData::<TestBlock>::V1 {
			blocks: vec![TestBlock::new(Header::new_from_number(1), vec![])],
			proof: CompactProof { encoded_nodes: vec![] },
		};
